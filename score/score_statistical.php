...
<body>
    <div class="content">
        ...
        <div class="chart-barjs">
            ...
            $query = "SELECT 
                    CAST(Diem as float) as FScore, -- Chuyển điểm thành kiểu số thực
                    COUNT(Diem) as SL -- Đếm số lượng điểm
                FROM DiemSo 
                WHERE MaBuoiHoc = '$maBH' -- Lọc theo buổi học
                AND NOT (
                    (Diem > 'a*' AND Diem < 'z*') -- Loại bỏ ký tự không phải số
        ...
        <table class="student-list" data-excel-name="Bảng điểm" style="width: 100%">
            ...
            while ($row = mysqli_fetch_array($result)) {
                ...
                echo '<form method="POST" action="/score/alter_score.php?MaLop=' . $maLop . '&MaBH=' . $maBH . '&MaPL=' . $row['MaPhanLop'] . '">';
                echo '<td style="padding: 0">';
                echo '<input name="AlterScore" id="iScore" type="text" value="' . $row['Diem'] . '" style="width: 50px;" onchange = "this.form.submit()">';
                echo '</td>';

                echo '</form>';

                // Xử lý tính toán Rank
                if (!is_numeric($row['Diem'])) {
                    ...
                    if ($rankRow = $results->fetch_assoc()) {
                        echo '<td>' . $rankRow['Rank'] . '</td>';
                    } else {
                        echo '<td>Không có thứ hạng</td>';
                    }
                }
                echo '</tr>';
            }
