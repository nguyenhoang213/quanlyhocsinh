                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        if (is_numeric($row['Diem'])) {
                            $maBH = $row['MaBuoiHoc'];
                            $maDS = $row['MaDiemSo'];
                            $sqlrank = "Select XepHang FROM (SELECT ds.MaBuoiHoc, ds.MaDiemSo, ds.Diem, DENSE_RANK() OVER (PARTITION BY ds.MaBuoiHoc ORDER BY ds.Diem DESC) AS XepHang FROM diemso ds WHERE ds.MaBuoiHoc = $maBH) as Ranking WHERE Ranking.MaDiemSo =  $maDS";
